    ...
    def forward(self,points, label = None):
        ...
        if self.training:
            if len(scores_anc):
                ...
                loss = loss + self.cls_loss(scores_anc,ancwise_label)
            ...
            if len(scores_anc):
                ...
                proposals = NMS_proposal(proposals,scores_anc)
...
class STD(nn.Module):
    ...
    def forward(self,proposals,xyz,features,label = None):
        ...
        proposal_points = in_side(xyz, features, proposals[:, :3], proposals[:, 3:6], proposals[:, 6])
        ...
        if self.training:
            ...
            iou_loss = smooth_l1(pred_iou[p_pos], p_iou_max[p_pos])

            closs = corner_loss(eps[p_pos], gt_eps[p_pos])

            box_loss = self.box_loss(pred_reg[p_pos], pred_cls[p_pos], reg, cls)

            cls_loss = self.cls_loss(score,p_pos.float())

            # loss = cls_loss + box_loss + iou_loss + closs

            return torch.cat([p_iou_max[p_pos],bbox_seven[p_pos]],dim = 1),cls_loss,box_loss,iou_loss,closs
        else:
